...
- **Priorities** — configurable levels (1 = Critical, 2 = High, …), not hard-coded; the New/Edit form's **color field is a swatch picker** — a 16-color preset grid plus a native color wheel and a hex input, all kept in sync with a hidden input so the chosen value is submitted like any field
- **Risks** — CRUD, `risk_score = probability × impact`, risk matrix, mitigation/contingency plans, owners
